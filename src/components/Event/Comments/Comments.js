...
class Comments extends Component {
  ...
  render() {
    ...
    for(let i = commentsArray.length - 1; i >= 0; i--){
      ...
      const comment = this.state.commenters.find(commenter => this.findCommenter(commenter, username))
      let image
      if (comment) {
        image = comment.image
      }
  ...
  deleteComment = (id) => {
    axios.delete(`/api/comments/delete-comment/${id}`)
    .then(this.loadComments())
    .catch(err => console.log(err));
  }

  loadComments = () => {
    axios.get(`/api/comments/get-comments/${this.state.eventId}`)
    .then((res) => {
  ...
  addCommentHandler = (comment) => {
    ...
    if (username && text) {
      ...
      axios.post(`/api/comments/post-comment/${this.props.eventId}`, COMMENT)
      .then(() => this.loadComments())
      .catch(err => console.log(err))
    }
  }
