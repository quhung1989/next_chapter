...
class Description extends Component {
   ...
   submitEdit = () => {
      ...
      axios
         .post("/api/users/post-user-data", data)
      ...
      if (this.state.editing) {
         description = (
            <Input
               ...
               eleType={descriptionState.eleType}
               eleConfig={descriptionState.eleConfig}
               invalid={!descriptionState.valid}
               value={descriptionState.value}
               changed={(event) => {
                  this.setState({
                     profile: inputChanger(
                        event,
                        "description",
                        this.state.profile,
                        descriptionState.validation.type
                     ),
                  });
               }}
            />
         ...
         buttons = (
            <div>
               <Button clicked={this.submitEdit}>Edit</Button>
               <Button clicked={this.toggleEdit}>Cancel</Button>
            </div>
         );
      } else {
         description = <div>{descriptionState.value}</div>;
         buttons = (
            <div>
               <Button clicked={this.toggleEdit}>Edit</Button>
            </div>
         );
      }
